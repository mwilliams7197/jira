	...
	if ticket != nil {
		...
		if existingTicket != nil {
			...
			if len(ticket.DeviceID()) > 0 {
				field := connector.GetFieldMap(backendDeviceID)
				if field != nil {
					updateBlock.Fields.DeviceID = ticket.DeviceID()
					oldToNewFieldName["deviceid"] = field.ID
				}
			}

			if len(sord(ticket.SystemName())) > 0 {
				field := connector.GetFieldMap(backendSystemName)
				if field != nil {
					updateBlock.Fields.SystemName = ticket.SystemName()
					oldToNewFieldName["systemname"] = field.ID
				}
			}
			...
			if len(sord(ticket.ApplicationName())) > 0 {
				...
				if field != nil {
					updateBlock.Fields.ApplicationName = ticket.ApplicationName()
